...
- 싱글턴 객체가 필요할 때 인스턴스를 달라고 요청하는 정적 메소드 ```getInstance()```가 있다. 

- **클래스 다이어그램**

![Singleton의 클래스 다이어그램](/headfirst-design-pattern-2207/singleton_pattern1.jpg)
- 클래스 변수에 싱글턴의 인스턴스가(**1개**) 저장된다.
...
: ```getInstance()``` 처리 순서에 따라 서로 다른 두 객체가 반환될 수 있다.
    ...
    - **해결 방법**
        ...
        - ```getInstance()```에  ```synchronized``` 키워드를 추가하면, 한 thread가 메소드 사용을 끝내기 전까지 다른 스레드는 기다려야 하기 떄문에 동시에 실행되지 않는다. (성능 저하 문제와 병목으로 작용할 수 있는 문제 때문에 <u>**메소드가 시작될 때만 효율적**이다.</u>)
